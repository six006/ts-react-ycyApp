import { ANativeRequest } from 'ts-react-app-native';
...
export class Request extends ANativeRequest {
    ...
    protected request<T>(type: string, url: string, postData?: tCommon.anyObject) {
        const p = f.AsyncOperation.createPromise<T>((resolve, reject) => {
            const bodyData = 'data=' + encodeURIComponent(JSON.stringify(this.sealPostData(postData)));
    ...
    userRegister(postData: {
        ...
        postData.source = f.Device.IsIOS ? 0 : 2;
